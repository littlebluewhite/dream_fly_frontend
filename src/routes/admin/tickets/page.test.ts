 ...
 * 已售/配額 line with a ProgressBar, and the desc. */
describe('票券管理 (+page)', () => {
	it('renders the PageHead title and 新增票券 action', () => {
	...
	it('renders the three summary StatCards (已售票券 / 票券營收 / 販售方案)', () => {
		const { container } = render(TicketsPage);
		const txt = container.textContent ?? '';
	...
	it('renders every ticket name from TICKETS', () => {
		const { container } = render(TicketsPage);
		const txt = container.textContent ?? '';
	...
	it('renders the ticket type badge labels (通行票 / 體驗票 / 活動票)', () => {
		const { container } = render(TicketsPage);
	...
	it("renders each ticket's price and 已售/配額 figures", () => {
		const { container } = render(TicketsPage);
		const txt = container.textContent ?? '';
	...
	it('renders a ProgressBar per ticket whose fill width matches soldPct', () => {
		const { container } = render(TicketsPage);
