<script lang="ts">
  /* 會員登入 Member Login — split-screen with social login. Ported from the
   * prototype (client/login.jsx + login.html). The `+page@.svelte` filename
   * resets to the ROOT layout, so this renders full-screen without the member
   * sidebar/topbar shell. The prototype's custom Field (icon input + password
   * toggle) and SocialButton aren't in the foundation, so they're inline markup
    ...
    <div style="width:100%; max-width:384px; display:flex; flex-direction:column; gap:24px;">
      ...
      <div style="display:flex; gap:12px;">
        <button
          ...
          on:click={submit}
          style="flex:1; height:48px; display:inline-flex; align-items:center;
        ...
        <button
          ...
          style="flex:1; height:48px; display:inline-flex; align-items:center;
            justify-content:center; gap:8px; border-radius:var(--df-radius-md); cursor:pointer;
            font-size:14px; font-weight:600; font-family:var(--df-font-body);
            background:#06C755; color:#fff; border:1.5px solid #06C755;"
        >
