<script context="module" lang="ts">
  import type { ToastTone } from '$lib/admin/stores';

  /* Tone → icon name, exported so the mapping can be asserted in a unit test
   * without rendering. Mirrors the member ToastStack / DS Toast. */
...
<script lang="ts">
  /* Bottom-right toast stack, driven by the admin `toasts` store. Title + body
