<script lang="ts">
   ...
   * POST /users：email/name/password 必填，phone 選填；密碼 8-128 字。前端在送出前
   * 同步擋 < 8 碼（不打一定會 422 的請求），錯誤顯示在密碼欄位的 inline hint（同
   * PasswordDialog 的 error 用法）。空白 phone 省略（undefined），對應 CreateUserRequest
   * 「不填 = 無電話」；不做 email 格式驗證——同 CouponCreateDialog 對其餘欄位的作法，
   * 交由後端 422 判斷。只有這一種模式（新增），沒有 isNew 分支——編輯是完全不同的
   * 欄位組合（name/phone/is_active，見 MemberEditDialog），兩者不共用同一個表單。
   * 本元件不打 API、不丟成功 toast——成功/失敗一律由呼叫端（members/+page.svelte）
   * 依 createMember() 的結果處理。 */
