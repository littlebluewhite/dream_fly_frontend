...
function makeFlatGate(initial: LoadPhase) {
	...
	// silentRefresh 是 Round 4 對 flat 閘門介面的擴充(寫入成功後靜默刷新)——fake 補齊
	// 型別即可,本元件測試不涉及它(retry 契約走 refresh)。
	const gate: LoadGate = { subscribe: store.subscribe, load, refresh, silentRefresh: vi.fn(() => Promise.resolve()), destroy: vi.fn() };
