...
export type { ApiOrderItem, ApiOrder } from '$lib/checkout-order';
 ...
 *  idempotencyKey 未指定時交由 submitOrder 產生新 uuid。回傳值改為
 *  `OrderConfirmation`（計畫核可的行為變更——原本回傳 raw ApiOrder；原始物件
 *  仍可經 `.raw` 取得）。 */
