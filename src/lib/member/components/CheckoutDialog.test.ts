...
describe('CheckoutDialog — 付款飛行中的關閉/重開競態（Idempotency-Key 防線）', () => {
	/* 階段 2 瘦身（arch R5 C5）：本 describe 只留兩個「顯示面」，競態的機器面移交
	 * checkout-controller.test.ts 的無渲染單元 its 接手——
	 *   ・重開不重置 paying/idempotencyKey →「付款飛行中外力關閉再重開：resumedInFlight」
	 *   ・再點不發第二張單／全程恰一張 →「alreadyPaying：飛行中重入立即返回」
	 *   ・失敗重試沿用同一把 key（原 render 從未斷言，淨增）→「失敗重試沿用同一把 key」 */

	/** 佈署一單「停在飛行中」的付款：POST /orders 回永不落定的 promise（resolve 之後
	 *  的機器面歸 controller 單元 its），並等到單子真的發出。 */
	async function renderInFlightPayment() {
		cart.addItem(COURSE);
		checkoutOpen.set(true);
		vi.mocked(api).mockImplementation(async (path: string, init: RequestInit = {}) => {
			const method = (init.method ?? 'GET').toString().toUpperCase();
	...
	it('付款飛行中 Escape / close() 不允許關閉', async () => {
		await renderInFlightPayment();
		await fireEvent.keyDown(window, { key: 'Escape' });
		expect(get(checkoutOpen)).toBe(true);
	});

	it('外部強制關閉再重開（不經 close()）：按鈕仍顯示「處理中…」——in-flight 狀態存活重開', async () => {
		const { getByText } = await renderInFlightPayment();
		checkoutOpen.set(false);
		await tick();
		checkoutOpen.set(true);
		await tick();
		expect(getByText('處理中…')).toBeInTheDocument();
	});
});
