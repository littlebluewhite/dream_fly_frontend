<script lang="ts">
  /* Toast host。ui.jsx ToastHost (173-190)。訂閱 stores 的 `toasts`。
   * Icon 用新式 lucide 名稱：success→circle-check / info→info / warning→triangle-alert /
   * error→circle-x / accent→star。 */
  import Icon from '$lib/components/ui/Icon.svelte';
  import { toasts, type ToastTone } from '$lib/mobile-admin/stores';
