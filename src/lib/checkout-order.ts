...
export async function submitOrder(
  ...
  opts: {
    coupon: string;
    usePoints: boolean;
    paymentMethod?: PaymentMethod; // 省略時 'credit_card'（與後端未帶時的預設一致）
  ...
  const order = await api<ApiOrder>('/orders', {
    method: 'POST',
    body: JSON.stringify({
      coupon_code: opts.coupon || undefined,
      use_points: opts.usePoints,
      payment_method: opts.paymentMethod ?? 'credit_card'
    }),
