<script context="module" lang="ts">
  import type { ToastTone } from '$lib/coach/stores';

  /* Tone → icon name, exported so the mapping can be asserted in a unit test
   * without rendering. Mirrors the member / admin ToastStack + DS Toast. */
...
<script lang="ts">
  /* Bottom-right toast stack, driven by the coach `toasts` store. Title + body
