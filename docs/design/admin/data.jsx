/* Dream Fly — Admin / Coach portal · shared data + helpers */

/* ---- Lucide icon helper ---- */
const Ic = ({ n, s = 20, c = "currentColor", style = {} }) =>
  React.createElement("i", { "data-lucide": n, style: { width: s, height: s, color: c, display: "inline-flex", flex: "none", ...style } });

/* Re-render lucide glyphs after every paint */
...
const PAY_STATUS = { paid: ["success", "已繳清"], due: ["warning", "待續費"], trial: ["info", "體驗中"] };
const ATT_MARK = { p: ["#10B981", "出"], a: ["#EF4444", "缺"], l: ["#F59E0B", "遲"], v: ["#94A3B8", "假"] };
...
const ORDER_STATUS = {
  paid: ["success", "已付款"], pending: ["warning", "待付款"], refunded: ["neutral", "已退款"],
};

/* ---- Today schedule (admin = all studio; coach = own) ---- */
const TODAY = [
...
/* coach (林雅婷) today */
const COACH_TODAY = [
  { time: "17:00", name: "競技體操 選手班", room: "A 訓練館", count: 12, state: "live", tone: "success", label: "進行中", taken: true },
  { time: "19:00", name: "競技啦啦隊 進階班", room: "A 訓練館", count: 11, state: "soon", tone: "warning", label: "即將開始", taken: false },
];

/* ---- Attendance roster — students of 競技啦啦隊 進階班 ---- */
const ROSTER = [
...
const MEMBER_STATUS = {
  active: ["success", "在學中"], warning: ["warning", "出席偏低"], paused: ["neutral", "暫停中"],
};

/* ===== 報表分析 data (lifted from .pen SEC/AdminBackend · 報表分析) ===== */
const REPORT_KPIS = [
...
/* revenue source drill-down rows */
...
const REVENUE_TOTAL = "NT$ 458,200";

/* 12-month revenue trend (NT thousands, relative bar heights) */
const REVENUE_TREND = [
...
/* top courses ranking (by enrolled) */
const TOP_COURSES = [
...
/* per-coach performance (revenue / students / attendance) */
const COACH_PERF = [
...
/* attendance-rate distribution (學員人數) */
const ATT_DIST = [
...
const TICKET_TYPE = { pass: ["primary", "通行票"], trial: ["success", "體驗票"], event: ["accent", "活動票"] };

/* ===== 加深內容 · 訂單 / 學員 / 班級 衰生欄位 ===== */
const CAMPUSES = ["美村本館", "文心分館", "北屯分館"];
const ENROLL_SOURCES = ["官網預約表單", "Facebook 廣告", "親友轉介", "Google 搜尋", "社區體驗活動", "LINE 官方帳號"];
function tierOf(pts) { return pts >= 350 ? ["金卡會員", "#F59E0B"] : pts >= 250 ? ["銀卡會員", "#94A3B8"] : pts >= 150 ? ["銅卡會員", "#B45309"] : ["一般會員", "#64748B"]; }

MEMBERS.forEach((m, i) => {
  if (m.campus) return;
  m.campus = CAMPUSES[i % CAMPUSES.length];
  m.source = ENROLL_SOURCES[(i * 2 + 1) % ENROLL_SOURCES.length];
  const by = 2026 - m.age;
  m.birthday = by + "/" + String((i * 5) % 12 + 1).padStart(2, "0") + "/" + String((i * 7) % 27 + 1).padStart(2, "0");
  const t = tierOf(m.points); m.tier = t[0]; m.tierColor = t[1];
  m.renewDue = m.pay === "trial" ? "體驗 06/30 到期" : m.pay === "due" ? ("已逾期 · " + ["05/28", "06/01", "06/03", "06/05"][i % 4]) : ("2026/" + ["09", "10", "11", "12"][i % 4] + "/15");
  m.lineId = "@df" + m.id.slice(-4);
});

ORDERS.forEach((o, i) => {
  if (o.campus) return;
  o.campus = CAMPUSES[i % CAMPUSES.length];
  o.tax = Math.round(o.amount - o.amount / 1.05);
  o.net = o.amount - o.tax;
  o.paidAt = o.status === "paid" ? o.date : o.status === "pending" ? "—（待付款）" : o.date;
  o.taxId = i % 5 === 0 ? "539012" + String(40 + i).slice(0, 2) : "—";
});

CLASSES.forEach((k, i) => {
  if (k.startDate) return;
  k.startDate = "2026/03/" + String((i % 27) + 1).padStart(2, "0");
  k.checkinRate = 86 + (i % 12);
  k.makeup = i % 3;
});

/* per-campus revenue (分校別營收) */
const CAMPUS_REVENUE = [
...
/* weekday class load + attendance (星期別課堂與出席) */
const WEEKDAY_LOAD = [
...
Object.assign(window, {
  Ic, useLucide, fmtNT,
  COACHES, CLASSES, MEMBERS, ORDERS, ORDER_STATUS, TODAY, COACH_TODAY, ROSTER, MESSAGES, ACTIVITY, MEMBER_STATUS,
  REPORT_KPIS, REVENUE_BREAKDOWN, REVENUE_TOTAL, REVENUE_TREND, CATEGORY_SPLIT, TOP_COURSES, INCOME_SOURCES,
  COACH_PERF, VENUE_USAGE, ATT_DIST, RETENTION, AGE_DIST, PAY_STATUS, ATT_MARK,
  VENUES, TICKETS, TICKET_TYPE,
  CAMPUS_REVENUE, PAYMENT_SPLIT, FUNNEL, WEEKDAY_LOAD, TIER_DIST, tierOf,
});
