...
- **mobile-admin 的種子退役是獨立後續任務，不在本批範圍**：`mobile-admin/data.ts` 的
  ...
  不現在預先動它。
- **Round 3（架構深化計畫稍後批次）會補其他候選項目的 ADR 條目定稿**：2026-07-10 架構審查
  的 8 個候選中，本 ADR 只記錄 C2（死種子退役）這一項落地的部分；其餘候選項目（含本輪
  Round 1 commit 2–4：`CartItem`/`Member` 型別上移、wire 乙線補完、`error-text.ts` 共同依賴
  預鋪）各自的決策記錄不在本 ADR 範圍。
