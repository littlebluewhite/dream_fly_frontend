...
- **`selectThread`**:`getThread`+`markRead` 雙 best-effort 平行 fetch,仿 attendance-controller
  的遞增 token 機制擋過期回應(`token !== seq` 即回傳 `stale`,不寫 `thread`);`markRead` 的
  `badgeCleared` 副作用不受此 guard 限制(逐字複刻頁面現行行為:已讀標記與目前是否還停留在該
  對話無關)。
...
非本 controller 收編範圍)——故以 outcome 欄位(`badgeCleared`)或頁面自行以呼叫當下捕捉的 id
交還頁面套用,不由 controller 直接改寫 `convos`。
...
`gate.silentRefresh()` 呼叫時機依判準④逐字留在各畫面;桌面「登入裝置」P2 區塊零觸碰。行動端經
`mobile-admin/api.ts` 零映射 re-export 取用 `createSettingsForm`/`SettingsDraft`。新增
`settings-form.test.ts` 七例,兩畫面既有 render 測試斷言面不變。
...
鏈本身的正確性由 `mobile-admin/api.test.ts`(經 `$lib/admin/api` 消費 `getReports`)另一路徑覆蓋。
...
- `src/lib/domain/sessions.ts` 落地後,倉內仍有三處測試 fixture 字面寫著舊值「進行中」:
  `admin/components/TodayPanel.test.ts`、`routes/admin/page.test.ts`(皆為直接餵入
  `TodayClass[]` prop 的展示字面,元件本身只渲染拿到的 `label` 字串,不經過
  `SESSION_STATUS` 查表)、以及 `mobile-admin/api.test.ts`(`getAdminHome` 描述區塊的 mock
  回應輸入值,同樣不經過真正的 `mapTodaySession`/`SESSION_STATUS` 映射路徑)。三處皆確認不影響
  測試綠燈、純屬觀感字面漂移,非本輪寫入集,留待未來若有人動到這些測試檔時順手更新,不需要為此
  單開任務。
- `CoachAvatar` 的 `initial` 預設值等既有 `docs/adr/0013` §2 記錄的漂移殘留不受本輪影響。

## 測試守衛

- `src/lib/coach/messages-controller.test.ts`(新檔,21 例):`selectThread` 的 stale-guard、
  `markRead` best-effort 與 `thread`/`sel` 無關、`send` 的 conversationId 快照防護與
  `sendFailed` 攜 `text`、compose 三態與 `creating` 守衛、`closeCompose` 專屬案例。
  `routes/coach/messages/page.test.ts` 保留 outcome→toast 映射與整合斷言。
- `src/lib/mobile-admin/overlays/CoachesScreen.test.ts`(新檔,7 例):`createAndRefresh`/
  ...
  `pendingUserId` 丟棄裁決。
- `src/lib/admin/settings-form.test.ts`(新檔,7 例):`applyData` 攤平、`maxClassSize`
  label↔數值換算、`save()` 三態 outcome、`alreadySaving` in-flight 守衛。兩畫面既有 render
  測試斷言面不變。
...
- `src/lib/admin/reports-api.test.ts`(新檔,285 行,原 `admin/api.test.ts` describe block
  逐字搬遷)+ `src/lib/admin/api.test.ts`(barrel 段零邏輯、既有斷言不變)+
  `src/lib/mobile-admin/api.test.ts`(三層 re-export 鏈的另一路徑覆蓋)。
