...
- store 層守衛——本 ADR「四個變體」一節明示屬 store 層、不在本次遷移範圍的 mobile-admin
  ...
  `refreshOps`／`refreshMessages`）原地改採這個 factory，mutator（`saveMember`／`saveClass`／
  `saveCoach`／`markOrderPaid`／`markMessageRead`）翻旗語意改經 `markMutated()`；guard 短路 +
...
- 呈現層 wrapper `src/lib/components/ui/LoadGate.svelte` 已於 2026-07-08 落地：把 44 個 route
  ...
  `refresh()` 一律真抓」契約在呈現層的落地方式。`load-gate.ts` 本身的公開介面仍依「後果」一節的
  凍結契約，未被本次呈現層收斂觸碰。
