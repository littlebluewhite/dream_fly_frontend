...
- **`silentRefresh` 不動 `phase`，只能從 settled 態呼叫**：分頁版另有 `silentRefresh()`——
  突變（新增/編輯/建立）後重新整包拉取最新分頁 meta，成功或失敗都不改變 `phase` 欄位（連
  `onError` 都不呼叫，失敗整個吞掉）。它假設呼叫當下閘門已經在一個 settled 態（`ready`/
  `error`），不是設計給與一個 in-flight 的 `load()` 交錯呼叫的情境——遷移到目前為止的用途
  （突變後重同步）天然滿足這個假設。
...
- **`load-gate.ts` 落地後即凍結為唯讀契約**：49 個呼叫端遷移不修改 `load-gate.ts` 本身的
  ...
  呼叫端。
- **`silentRefresh` 假設呼叫時機在 settled 態**：如果在一個 in-flight 的 `load()` 進行中呼叫
  `silentRefresh()`（或反之），`phase` 可能停留在呼叫前的值而不會正確反映最新一次請求的結果
  ——這是規格層面的既知行為，不是缺陷；目前所有呼叫點（突變後重同步）都天然只在 `ready` 態
  觸發，不受影響。
