...
（`GET /reports/admin`）在本 ADR 定稿前即已接上真實 API，不屬於本節「demo/mock」之列。
...
| admin dashboard 今日／動態 | `src/lib/admin/data.ts`（`TODAY`/`ACTIVITY`） | 首頁「今日課表」／「最新動態」仍是 live-mock，後端無對應端點；原始碼已加註 P2 標記並指回本 ADR |
