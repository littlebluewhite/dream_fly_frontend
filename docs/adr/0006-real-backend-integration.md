...
因此不再是 mock-only，而是被 member 真結帳與 public 購物車頁共同消費的純數學模組。admin 的
`ClassEditDialog`（`coaches`）與 `OrdersTable`（`rows`）也已移除 mock 時代的預設值、改為必填
prop，由頁面（`classes`/`orders`）傳入真實資料（2026-07-07 更新）。
