...
## 增補（2026-07-23，架構深化 R8 C1）：messages-controller 是單頁 controller 第六例

`src/lib/coach/messages-controller.ts`（`createMessagesController`）把 `coach/messages/+page.svelte`
的對話串載入（`selectThread`，內部化 `getThread`+`markRead` 雙 best-effort fetch 與 token
stale-guard）、傳送（`send`，過期回應防護改採 conversationId 快照比對）、撰寫新對話三部曲
（`openCompose`/`pickRecipient`/`confirmCompose`，`creating` 守衛）收進單頁 controller，四判準
逐條核對全數成立。對話清單、其 gate、tab×搜尋過濾（`conversations-filter.ts`，K3，不被取代）、
