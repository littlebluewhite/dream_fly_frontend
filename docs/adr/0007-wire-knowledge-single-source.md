...
### `orderIdentity` / `taxFromGross`：訂單雙身分協定與 5% 內含稅顯示反推（2026-07-20 R5 C7 增收）

依同一收錄判準（≥2 surface 共用的後端 wire 知識）增收兩支零 import 純函式：

- **`orderIdentity(o) → { display, uuid }`**——訂單雙身分協定的單點：顯示/單號用 `order_number`、
  `PATCH /orders/{id}/status` 用真 uuid。先前這條協定散在 admin 的 `mapAdminOrder` 與呼叫端記憶裡；
  現況四處 wire interface（含 PATCH 回應形 `ApiOrderStatusResponse`）的 `order_number` 皆必填，
  無 fallback、不虛構。member `mapOrder` 同步實接（`id: orderIdentity(o).display`，綁契約、grep 可尋）。
- **`taxFromGross(amount) → { tax, net }`**——`tax = Math.round(amount - amount / 1.05)`，5% 內含稅
  的**顯示層推導**（稅非後端欄位，P2 註記）；**單位跟隨傳入值**（兩呼叫端皆傳 NT$ 整數——admin
  mapper 先經 `ntd()` 換算才呼叫）。站點級 pin：admin 480000 分 fixture 斷 `tax===229 && net===4571`，
  mobile-admin `ORDERS[0]` 同款。

檔頭「凍結」語指當年批次的任務邊界，非永久禁令——本增量即依判準走完同一道門。`mapOrderRow` 仍不做
（admin Order 18 欄 vs member Order 5 欄是 per-surface UI 目標型別，ADR 0003/本篇不收清單既有裁決）。

### 明確不收清單（防未來誤收）

- `ntd` / `toCents` / `orderItemsSummary`——留在 `public/adapters.ts`，是 ADR 0006 訂下的
  cents↔NT$ 換算邊界，屬於「換算規則」而非「wire 形狀知識」；`taxFromGross` 收的是稅式推導，
  不觸這條換算邊界（單位跟隨呼叫端）。
...
- mobile 雙 surface 的 tuple 型 `Tone`（`[string, string]`）及其 3 值子集——這是 `mobile`/
  `mobile-admin` 各自 `data.ts` 的本地展示層概念，與頁面是否已接上真後端無關（兩者已於 Round 3
  Task 19/20 接上真後端，見 ADR 0006）；若之後發現這塊型別與 `wire.ts` 既有型別重複，再一併
  評估是否收斂，本次整合刻意不預先處理。
...
- `mobile-admin` 已於 Round 3（Task 20）接上真後端（見 ADR 0006 §5）；若之後發現 tuple 型
  `Tone` 或其他 wire 形狀與 `wire.ts` 既有型別重複，再評估收斂；本次整合刻意不預先處理。
