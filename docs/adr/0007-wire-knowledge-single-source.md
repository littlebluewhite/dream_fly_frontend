...
- **已知殘留**：`coach/api.ts` 的 `mapCoach()` 仍有一處 `user.last_login.slice(0, 16)
  .replace('T', ' ')` 內聯字面重複 `isoDateTime()` 的邏輯，未委派到 `wire.ts`——8 檔改線後
  唯一一處「字面合規、實質仍是複製」的殘留，留待下次動到 `coach/api.ts` 時一併收斂，非本次
  範圍。
